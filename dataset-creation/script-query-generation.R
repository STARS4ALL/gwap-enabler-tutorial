...
# 3) SQL query generation
    ...
    write(query.resource, file="resource-insert.sql")  # all the resource INSERT queries
    ...
    write(query.topic, file="topic-insert.sql") # all the topic INSERT queries
    ...
    # c) table RESOURCE HAS TOPIC (per GT solo una riga, per le altre una riga per ogni categoria)
      ...
      for(t in 1:nrow(gt)){
        qgt<-paste0("INSERT INTO resource_has_topic (idResource, idTopic, score) VALUES ((SELECT idResource FROM resource WHERE refId =", gt$X.id[t], "), (SELECT idTopic FROM topic WHERE refId ='", gt$cuisine[t], "'), 2);")
      ...
      for(w in 1:nrow(to.classify)){
        for(f in 1:nrow(categories)){
          qtc<-paste0("INSERT INTO resource_has_topic (idResource, idTopic, score) VALUES ((SELECT idResource FROM resource WHERE refId =", to.classify$X.id[w], "), (SELECT idTopic FROM topic WHERE refId ='", categories$label[f], "'), 0);")
